...
function processRequest(req, res) {
  ...
  proxyReq.on('error', (e) => {
    ...
    totalErrors++;
    recordUpstreamError(upstreamUrl.hostname);
    try { res.write(JSON.stringify({ _relay: { status: 502, headers: {}, error: 'upstream error: ' + e.message } }) + '\n'); } catch {}
    try { res.end(); } catch {}
  });
  ...
  proxyReq.on('timeout', () => {
    ...
    if (!aborted) {
      recordUpstreamError(upstreamUrl.hostname);
      try { res.write(JSON.stringify({ _relay: { status: 504, headers: {}, error: 'upstream timeout' } }) + '\n'); } catch {}
      try { res.end(); } catch {}
    }
  });
